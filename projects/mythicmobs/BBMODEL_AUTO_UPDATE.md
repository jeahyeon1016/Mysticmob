...
4. 최신 ZIP을 PaperResources가 읽는 GitHub 저장소에 커밋·푸시한다. 현재 서버의 URL은 `Mysticmob` 저장소 `main` 브랜치를 가리키므로, 이 URL을 유지하는 한 ZIP은 `jeahyeon1016/Mysticmob`에 반영한다. 저장소·브랜치·경로가 다르면 `bukkit/plugins/PaperResources/resources.txt`의 실제 URL을 먼저 확인한다.

   ```text
   D:\ai agent\projects\mythicmobs\bukkit\plugins\ModelEngine\resource pack.zip
   ```

   `bukkit/plugins/PaperResources/resources.txt`의 URL이 실제 GitHub Raw ZIP을 가리키는지 확인한다. Paper 서버 `server.properties`의 `resource-pack`이 비어 있어도 PaperResources가 URL을 관리할 수 있다. 별도로 `resource-pack-sha1`을 사용하는 배포라면 로컬 ZIP SHA-1과 원격 ZIP SHA-1을 일치시킨다.
